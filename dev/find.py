...
def parse_image_dir_path(path, quiet=True):
    ...
    try:
        # Use os.path.normpath and os.sep to handle platform-specific separators
        normalized_path = os.path.normpath(path)
        parts = normalized_path.split(os.sep)
        mode, dataset_name, dirname = parts[-3:]
    except ValueError:
    ...
    if mode == "attacked":
        ...
        if not dirname in ["real", "stable_sig", "stegastamp", "tree_ring", "wam"]:
            raise ValueError(
                "Source name must be one of ['real', 'stable_sig', 'stegastamp', 'tree_ring', 'wam']"
            )
        source_name = dirname
        if not quiet:
            print(" -- Dataset name:", dataset_name)
